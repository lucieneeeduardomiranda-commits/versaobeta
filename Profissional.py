...
st.set_page_config(page_title="Scanner Pro v8.1 - Calibrado (EV > 0.10)", page_icon="⚽", layout="wide")
...
def calcular_odd_minima_para_ev(probabilidade_ganho, probabilidade_perda, ev_alvo=0.10):
    ...
    # Em mercados com push, a odd mínima considera apenas ganho e perda efetiva
...
st.title("⚽ Analisador de Escanteios v8.1")
...
if st.button("CALCULAR ANÁLISE PROFISSIONAL"):
    ...
    # 2. Distribuição de Probabilidade (Binomial Negativa k=3.0)
    k_fixed = 3.0
    distribuicao = [neg_binomial_prob(k, lambda_r, k_fixed) for k in range(41)]
    
    # 3. Definição de Probabilidades Over/Under/Push
    cantos_faltam_linha = linha - atuais
    is_half = (linha * 10) % 10 != 0

    if is_half:
        p_under = sum(distribuicao[k] for k in range(len(distribuicao)) if k < cantos_faltam_linha)
        p_over = 1.0 - p_under
        p_push = 0.0
    else:
        # Ex: Linha 10.0, atuais 5 -> faltam 5.0. 
        # k < 5 (perda), k == 5 (reembolso), k > 5 (vitória)
        idx_push = int(cantos_faltam_linha)
        p_under = sum(distribuicao[k] for k in range(idx_push) if k < len(distribuicao))
        p_push = distribuicao[idx_push] if idx_push < len(distribuicao) else 0
        p_over = 1.0 - p_under - p_push

    # 4. Cálculo de EV Real (Considerando que Push não gera lucro nem prejuízo)
    # EV = (P_vitória * Lucro) - (P_derrota * Stake)
    ev_o = (p_over * (odd_o - 1)) - (p_under * 1)
    ev_u = (p_under * (odd_u - 1)) - (p_over * 1)
    ...
    with col_res2:
        ...
        entrada_identificada = False

        # Filtro Rigoroso: Só entra se EV >= 0.10
        if ev_o >= 0.10:
            f_k = calcular_kelly(p_over, p_under, odd_o, agressividade)
            if f_k > 0:
                st.success(f"**ENTRADA: OVER {linha}**")
                st.write(f"Stake Sugerida: **R$ {banca_total * f_k:.2f}**")
                st.caption(f"({f_k:.2%} da banca)")
                entrada_identificada = True

        elif ev_u >= 0.10:
            f_k = calcular_kelly(p_under, p_over, odd_u, agressividade)
            if f_k > 0:
    ...
    odd_min_over = calcular_odd_minima_para_ev(p_over, p_under, ev_alvo=0.10)
    odd_min_under = calcular_odd_minima_para_ev(p_under, p_over, ev_alvo=0.10)
    
    col_odd1, col_odd2 = st.columns(2)
    with col_odd1:
        if odd_min_over: st.metric(f"Mínima Over {linha}", f"{odd_min_over:.2f}")
    with col_odd2:
        if odd_min_under: st.metric(f"Mínima Under {linha}", f"{odd_min_under:.2f}")