...
with st.sidebar:
    ...
    agressividade = st.slider("Fator Kelly", 0.05, 0.5, 0.2)
...
if st.button("CALCULAR ANÁLISE COM MOMENTUM"):
    ...
    with col_res1:
        ...
        ev_col1.metric("EV Over", f"{ev_o:.3f}", delta=f"{(ev_o*100):.1f}%")
        ev_col2.metric("EV Under", f"{ev_u:.3f}", delta=f"{(ev_u*100):.1f}%")
