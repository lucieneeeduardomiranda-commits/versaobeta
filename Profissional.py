...
# --- DICIONÁRIO DE LIGAS (K e Médias) ---
# Organizado para evitar erros de referência 
LIGAS_DATA = {
...
with st.sidebar:
    ...
    # 1. Primeiro definimos a seleção da liga 
    liga_selecionada = st.selectbox("Selecione a Liga", list(LIGAS_DATA.keys()))
    
    # 2. IMEDIATAMENTE recuperamos os dados para evitar o NameError 
    dados_liga = LIGAS_DATA[liga_selecionada]
    
    # 3. Agora usamos os dados recuperados nos componentes 
    k_ajustado = st.slider("Ajuste de Dispersão (K)", 0.5, 4.0, float(dados_liga["k"]), 0.01)
    ...
    # Média preenchida automaticamente com base na seleção [cite: 8]
    media_liga = st.number_input("Média de Escanteios da Liga", 5.0, 20.0, float(dados_liga["media"]), 0.01)
    ...
    linha = st.number_input("Linha de Aposta (Over)", 0.5, 25.0, float(atuais + 2.5), 0.5)
...
if st.button("CALCULAR ANÁLISE COM MOMENTUM"):
    ...
    with col_res1:
        ...
        # Delta visível apenas para EV entre 0.10 e 0.40 
        ev_col1.metric("EV Over", f"{ev_o:.3f}", delta=f"{(ev_o*100):.1f}%" if 0.10 <= ev_o < 0.40 else None)
        ev_col2.metric("EV Under", f"{ev_u:.3f}", delta=f"{(ev_u*100):.1f}%" if 0.10 <= ev_u < 0.40 else None)

    with col_res2:
        st.subheader("💰 Stake Sugerida")
        limite_bloqueio = 0.40 # Trava de segurança solicitada
        ...
        if 0.10 <= ev_o < limite_bloqueio:
            ...
            st.write(f"Entrada bloqueada: EV superior a {limite_bloqueio:.2f}.")
        else:
    ...
    with col_odd2:
        if odd_min_under: st.metric(f"Mínima Under {linha}", f"{odd_min_under:.2f}", f"Atual: {odd_u:.2f}")
